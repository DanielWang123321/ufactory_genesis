...
Usage:
    ...
    python examples/bio_gripper/view_bio_gripper_movable.py --headless
"""
...
from ufactory.paths import bio_gripper_movable_visual_urdf
...
def main() -> None:
    ...
    robot = scene.add_entity(
        gs.morphs.URDF(file=bio_gripper_movable_visual_urdf(), fixed=True),
        surface=glb_view_surface(),
    )
    scene.build()

    drive_idx, all_idx = bio_gripper_dof_indices(robot)
    setup_bio_gripper_pd(robot, drive_idx, all_idx)

    step = 0
    while True:
        q = bio_gripper_demo_target(step)
        control_bio_gripper_pose(robot, drive_idx, all_idx, q)
        scene.step()
        step += 1
