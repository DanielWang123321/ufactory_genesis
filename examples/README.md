...
The grasp-place real path is enabled with `--executor servo_cartesian` or `--executor servo_j`. `servo_cartesian` streams Cartesian targets and lets firmware solve IK. `servo_j` compiles every Cartesian tick with host-side Genesis IK, preserves the same tick count and duration, then streams explicit joint targets. `--speed-mm-s` and `--mvacc-mm-s2` control the shared MoveL source timing for sim, dry-run, and real streaming; defaults are 150 mm/s and 800 mm/s^2. Both executors default to dry-run mode, which prints segment and per-tick safety summaries and does not move the real robot:
