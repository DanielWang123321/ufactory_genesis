"""
Standalone Lite6 gripper movable visual demo (no robot arm).

Usage:
    export NUMBA_CACHE_DIR=~/.cache/numba
    python examples/lite6_gripper/view_lite6_gripper_movable.py
    python examples/lite6_gripper/view_lite6_gripper_movable.py --headless
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Lite6 gripper open/close demo (standalone)")
    ...
    robot = scene.add_entity(
        gs.morphs.URDF(file=lite6_gripper_movable_visual_urdf(), fixed=True),
    ...
    step = 0
    while True:
        q = lite6_gripper_demo_target(step)
        control_lite6_gripper_pose(robot, drive_idx, all_idx, q)
        scene.step()
        step += 1
