...
BIO_GRIPPER_OPEN = 0.0
BIO_GRIPPER_CLOSE = 0.04
BIO_GRIPPER_HOLD_STEPS = 200
...
def bio_gripper_dof_indices(robot) -> tuple[list[int], list[int]]:
  joint_map = {j.name.split("/")[-1]: j for j in robot.joints}
  if "right_finger_joint" not in joint_map:
    return [], []
  drive_idx = [joint_map["right_finger_joint"].dofs_idx_local[0]]
  all_idx = [joint_map[n].dofs_idx_local[0] for n in BIO_GRIPPER_JOINTS if n in joint_map]
...
def _targets_for_active(all_dof_idx: list[int], right_value: float, robot) -> np.ndarray:
  ...
  for dof_i in all_dof_idx:
    for name in BIO_GRIPPER_JOINTS:
      if name in joint_map and joint_map[name].dofs_idx_local[0] == dof_i:
        values.append(right_value if name == "right_finger_joint" else -right_value)
        break
  return np.array(values)


def setup_bio_gripper_pd(robot, gripper_dof_idx: list[int], all_gripper_dof_idx: list[int]) -> None:
...
def bio_gripper_demo_target(step: int) -> float:
  phase = (step // BIO_GRIPPER_HOLD_STEPS) % 2
  return BIO_GRIPPER_CLOSE if phase else BIO_GRIPPER_OPEN
