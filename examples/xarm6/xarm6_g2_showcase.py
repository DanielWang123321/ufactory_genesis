...
Solid dark green table, xArm6 on the table long edge, physics grasp of a red block, place into an open cardboard shipping box.
...
except ModuleNotFoundError:  # Imported as examples.xarm6.xarm6_g2_showcase.
    from examples import _bootstrap  # type: ignore[no-redef]  # noqa: F401
...
try:
    ...
    from examples._packaging_scene import (
        FINGER_FRICTION_MU,
        HOME_RPY_DEG,
        ROBOT_BASE_YAW_DEG,
        build_packaging_scene,
...
from ufactory.trajectory.packaging import build_packaging_program, packaging_layout
...
def _setup_robot(robot, layout, *, arm_kp_scale: float | None = None):
    runtime = get_robot_runtime_profile("xarm6")
    gripper = runtime.gripper
    if gripper is None:
        raise RuntimeError("xArm6 packaging showcase requires Gripper G2")
    ik_link = robot.get_link(runtime.arm.ee_link)
    left_finger = robot.get_link("left_finger")
    right_finger = robot.get_link("right_finger")
    ...
    opening_gripper_dof_idx = [gripper_dof_idx[0], all_gripper_dof_idx[3]]
    applied_arm_kp_scale = layout.simulation_arm_kp_scale if arm_kp_scale is None else float(arm_kp_scale)
    robot.set_dofs_kp(
        torch.tensor(
            np.asarray(runtime.arm.kp) * applied_arm_kp_scale,
            device=gs.device,
            dtype=gs.tc_float,
        ),
        arm_dof_idx,
    ...
    robot.set_dofs_kp(
        torch.full((2,), gripper.kp, device=gs.device, dtype=gs.tc_float),
        opening_gripper_dof_idx,
    )
    robot.set_dofs_kv(
        torch.full((2,), gripper.kv, device=gs.device, dtype=gs.tc_float),
        opening_gripper_dof_idx,
    )
    robot.set_dofs_force_range(
        torch.full((2,), gripper.force_lower, device=gs.device, dtype=gs.tc_float),
        torch.full((2,), gripper.force_upper, device=gs.device, dtype=gs.tc_float),
        opening_gripper_dof_idx,
    )
...
def init_showcase_robot(
    ...
    *,
    ...
    home_qpos_saved = _init_home_qpos(robot, ik_link, arm_dof_idx, all_gripper_dof_idx, down_quat, home_pos)
    scene.step()
...
def hold_robot_home(robot, scene, ctx: ShowcaseRobotCtx, *, steps: int = 1) -> None:
    ...
    gripper_open = torch.full((1, 1), GRIPPER_OPEN, device=gs.device, dtype=gs.tc_float)
...
def _with_sim_release_timing(program, *, speed: float):
    """Return a copy with the packaging-sim-only preload release and timing."""
    speed_scale = max(0.25, float(speed))
    duration = SIM_RELEASE_DURATION_S / speed_scale
    relax_duration = SIM_PRE_RELEASE_RELAX_DURATION_S / speed_scale
    segments = []
    for segment in program.segments:
        if segment.label == "pre-release-settle":
            ...
            tuned = replace(
                ...
                gap_end=SIM_PRE_RELEASE_RELAX_GAP_M,
                duration=relax_duration,
            )
        elif segment.label == "release":
            tuned = replace(
                segment,
                gap_start=SIM_PRE_RELEASE_RELAX_GAP_M,
                duration=duration,
            )
        else:
            segments.append(segment)
...
def _speed_config(speed: float, config_path: Path | None = None):
    """Load the versioned task with a simulation-only playback multiplier."""
    mult = max(0.25, float(speed))
    base = load_runtime_config("xarm6", task="packaging_showcase", config_path=config_path)
...
def _trajectory_context(
    ...
    *,
    ...
    return TrajSceneContext(
        ...
        gripper_open_dof_idx=[ctx.all_gripper_dof_idx[0], ctx.all_gripper_dof_idx[3]],
        gripper_initial_open_dof_idx=list(ctx.all_gripper_dof_idx),
        gripper_initial_open_clearance_m=0.003,
        visual_model="glb",
...
def run_pick_place_cycle(
    ...
    *,
    ...
    config = _speed_config(speed, config_path)
    task_layout = packaging_layout(config)
    ...
    program = _with_sim_release_timing(program, speed=speed)
    ...
    ok = report.place_error_mm < task_layout.place_success_distance_m * 1000.0 and report.home_drift_mm < 10.0
    print(
...
def _cycle_failure_reason(report: SimReport, layout, task_layout) -> str | None:
    ...
    min_lift_z_mm = (layout.table_top_z + 0.100) * 1000.0
    ...
    if report.home_drift_mm >= 10.0:
        return f"home drift {report.home_drift_mm:.1f} mm exceeds 10.0 mm"
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    runtime_config = load_runtime_config("xarm6", task="packaging_showcase", config_path=args.config)
    ...
    try:
        ...
        from examples._robot_viewer import start_deferred_viewer
