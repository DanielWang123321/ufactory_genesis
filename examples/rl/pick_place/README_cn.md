...
参考环境为 Linux、Genesis 支持的 NVIDIA GPU、Python 3.12/3.13、Genesis World 1.3.1、
...
在 v0.2.12 的 Genesis 1.3.1 物理基线上，权重不变的随仓库策略在种子 `1/7/17` ×
并行环境 `1/8/64` 的九组无扰动测试中为 9/9（合计 219/219），独立固定场景库
64 回合为 64/64。

动作高斯扰动标准差为 `0.02`、固定种子为 `20260731` 时，结果为
**512/512 完整成功且质量通过**；最终平面误差、首次夹起前拖动、松爪后漂移的
P99 分别为 8.35、1.05、0.73 mm，动作裁剪和 IK 故障均为零。诊断仍记录到
120/512 回合发生松爪后手指重接触；该诊断不属于稳健性聚合门的判定条件，因此这里
不宣称扰动下完全无接触，也不外推为真机稳健性。精确工件元数据见
`pretrained/evaluation_summary.json`。
