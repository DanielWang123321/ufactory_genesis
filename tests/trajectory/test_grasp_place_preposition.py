...
def test_sdk_sim_feedback_prepositions_only_virtual_robot(monkeypatch):
    monkeypatch.setattr("ufactory.cli.grasp_place.time.sleep", lambda _seconds: None)
...
def test_default_grasp_place_layout_matches_v024_sequence():
    config = load_runtime_config("xarm6")
...
def test_lite6_grasp_place_program_has_place_settle_not_grip_settle():
...
def test_default_grasp_place_program_preflight_passes_servo_j():
    pytest.importorskip("pinocchio")
    pytest.importorskip("genesis")
    from ufactory.cli.grasp_place import _compile_servo_j_program
...
def test_lite6_cartesian_preflight_preserves_gripper_down_orientation():
    ...
    from ufactory.cli.grasp_place import _backends, _model_and_hashes
