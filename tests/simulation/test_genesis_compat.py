"""Version and private-hook contract tests for the Genesis 1.3.1 baseline."""
...
def test_version_below_minimum_is_rejected(monkeypatch):
    ...
    with pytest.raises(compat.GenesisCompatibilityError, match=r"Genesis>=1\.3\.1 is required"):
...
def test_newer_version_warns_only_once(monkeypatch):
    ...
    with pytest.warns(RuntimeWarning, match="only 1.3.1 is the project's reference baseline") as caught:
        compat.require_genesis_version()
        compat.require_genesis_version()
    assert len(caught) == 1
...
def test_installed_genesis_matches_runtime_and_hook_contracts():
    ...
    if str(version) != "1.3.1":
        pytest.skip("the full installed-contract assertion targets the reference 1.3.1 baseline")
