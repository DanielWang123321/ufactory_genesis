"""Version and private-hook contract tests for the Genesis 1.3.0 baseline."""
...
def test_version_below_minimum_is_rejected(monkeypatch):
    ...
    with pytest.raises(compat.GenesisCompatibilityError, match=r"Genesis>=1\.3\.0 is required"):
        compat.require_genesis_version()


def test_validated_version_is_accepted_without_warning(monkeypatch):
    monkeypatch.setattr(compat.metadata, "version", lambda _name: "1.3.0")
...
def test_newer_version_warns_only_once(monkeypatch):
    ...
    with pytest.warns(RuntimeWarning, match="only 1.3.0 is the project's reference baseline") as caught:
        compat.require_genesis_version()
        compat.require_genesis_version()
    assert len(caught) == 1
...
def test_installed_genesis_matches_runtime_and_hook_contracts():
    ...
    if str(version) != "1.3.0":
        pytest.skip("the full installed-contract assertion targets the reference 1.3.0 baseline")
