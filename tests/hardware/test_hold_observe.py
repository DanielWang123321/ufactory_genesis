...
from ufactory.hardware.observe import analyze_hold_timeseries
from ufactory.hardware.session import HoldTimeSeriesSample, RealRobotSession
...
def test_collect_hold_timeseries_sample_count(monkeypatch):
    ...
    monkeypatch.setattr("ufactory.hardware.session.time.monotonic", fake_monotonic)
    monkeypatch.setattr("ufactory.hardware.session.time.sleep", fake_sleep)
